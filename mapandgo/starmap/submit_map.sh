...
echo "/hpc/hub_oudenaarden/aalemany/bin/map/starmap/map.sh $1 $2 $3 $4 $5 $6" | qsub -cwd -N star-${outfq} -o star-${outfq}.out -e star-${outfq}.err -m eas -M ${email} -pe threaded ${threads} -l h_rt=${Dt} -l h_vmem=${Dmem} -V
